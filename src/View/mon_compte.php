...
<body class="bg-gray-100 min-h-screen flex">
    ...
    <main class="flex-1 flex items-center justify-center p-6">
        <div class="w-full max-w-lg bg-white rounded-lg shadow-lg p-8">
            <h2 class="text-3xl font-bold mb-6 text-center text-gray-800">Mon compte</h2>
            ...
            <?php if ($user): ?>
                <form method="post" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="<?php echo htmlspecialchars($user->getRole()); ?>" class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-100" disabled>
                    </div>
                    <div>
                        <label for="createdAt" class="block mb-1 font-medium text-gray-700">Créé le</label>
                        <input type="text" value="<?php $date = $user->getCreatedAt(); if ($date) { $dt = new DateTime($date); echo $dt->format('d/m/Y à H:i'); } else { echo '-'; } ?>" class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-100" disabled>
                    </div>
                    <div>
                ...
                <form method="post" action="index.php?page=supprimer_compte" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer votre compte ? Cette action est irréversible.');">
                    <button type="submit" class="w-full bg-red-600 text-white py-2 rounded font-semibold hover:bg-red-700 transition mt-4">Supprimer mon compte</button>
                </form>
            <?php else: ?>
